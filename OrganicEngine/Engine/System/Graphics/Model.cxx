...
#include "../../../Resource/assimp/Importer.hpp"
#include "../../../Resource/assimp/scene.h"
#include "../../../Resource/assimp/postprocess.h"

#ifdef DEBUG
#pragma comment(lib, "Resource/assimp/x64/Debug/assimp-vc143-mtd.lib")
#else
#pragma comment(lib, "Resource/assimp/x64/Release/assimp-vc143-mt.lib")
#endif

using namespace Engine::Graphic;

std::shared_ptr<iVertexShader> Model::m_defVS = nullptr;
std::shared_ptr<iPixelShader> Model::m_defPS = nullptr;
...
void Model::SetVertexShader(const std::string& Tag)
{
	m_pVS = ShaderManager::GetInstance().LoadVertexShader(Tag);
}
void Model::SetPixelShader(const std::string& Tag)
{
	m_pPS = ShaderManager::GetInstance().LoadPixelShader(Tag);
}
...
void Model::SetTransform(Engine::Math::Transform* tf)
{
	m_transform = tf;
}
Engine::Math::fVec4x4 Model::GetWorldMatrix()
{
	Engine::Math::fVec4x4 ret;
	DirectX::XMStoreFloat4x4(&ret, m_transform->GetWorldMatrix());
	return ret;
}
...
bool Model::Load(const char* file, float scale, bool flip)
{
	...
	for (unsigned int i = 0; i < pScene->mNumMeshes; ++i)
	{
		...
		mesh.mesh = CreateMeshBuffer(desc);
	...
	for (unsigned int i = 0; i < pScene->mNumMaterials; ++i)
	{
		...
		if (pScene->mMaterials[i]->Get(AI_MATKEY_TEXTURE_DIFFUSE(0), path) == AI_SUCCESS) {
			// そのまま探索
			material.texture = TextureManager::GetInstance().CreateTexture(path.C_Str());
...
void Model::RemakeVertex(int vtxSize, std::function<void(RemakeInfo& data)> func)
{
	...
	while (it != m_meshes.end())
	{
		Description desc = it->mesh->GetDesc();
...
float4 main(PS_IN pin) : SV_TARGET
{
	...
	m_defVS = ShaderManager::GetInstance().LoadVertexShader("Model_Def");
	m_defVS->Compile(vsCode);
	m_defPS = ShaderManager::GetInstance().LoadPixelShader("Model_Def");
